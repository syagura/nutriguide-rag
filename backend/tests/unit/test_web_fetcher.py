...
def test_fetch_page_returns_html_text_on_success(mock_get, mock_robots):
    ...
    content, content_type = fetch_page("https://who.int/artikel")
    assert content == "<html>...</html>"
    assert content_type == "text/html"

@patch("src.core.services.web.fetcher._is_allowed_by_robots", return_value=True)
@patch("src.core.services.web.fetcher.requests.get")
def test_fetch_page_returns_pdf_bytes_on_success(mock_get, mock_robots):
    mock_response = MagicMock()
    ...
    content, content_type = fetch_page("https://who.int/laporan.pdf")
...
@patch("src.core.services.web.fetcher._is_allowed_by_robots", return_value=True)
@patch("src.core.services.web.fetcher.requests.get")
def test_fetch_page_returns_none_on_unsupported_content_type(mock_get, mock_robots):
    mock_response = MagicMock()
    mock_response.headers = {"Content-Type": "application/json"}
    mock_response.raise_for_status.return_value = None
    mock_get.return_value = mock_response

    assert fetch_page("https://who.int/data.json") is None
...
def test_fetch_pages_skips_failed_urls(mock_fetch_page, mock_sleep):
    mock_fetch_page.side_effect = [("<html>ok</html>", "text/html"), None, (b"pdfbytes", "application/pdf")]
    pages = fetch_pages(["url1", "url2", "url3"])
    assert pages == {"url1": ("<html>ok</html>", "text/html"), "url3": (b"pdfbytes", "application/pdf")}